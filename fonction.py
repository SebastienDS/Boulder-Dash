...
def test_ouverture_custom_map():
	...
	:return: return le nom si oui sinon return None
	"""
...
def recup_pos_diamant_requis(carte):
    """
	recherche des positions vers les diamants

	:param list carte: map en cours de jeu
	:return: liste des positions vers les diamants
	"""
    list_pos_diams = []
    for j in range(len(carte)):
        for i in range(len(carte[0])):
...
def recherche_parcours_vers_position(carte, depart, fin):
    """
	:param list carte: map en cours de jeu
	:param tuple depart: position de depart du personnage
	:param tuple fin: postion final du personnage
	:return: renvoie si un chemin a été trouvé
	"""
    M = [
        [
            None
            ...
            in {
                elem
                for elem in {".", "G", "L", "D", "R", "E"}
                if elem != "E" or fin == (var["pos_sortie_x"], var["pos_sortie_y"])
            }
            else "WW"
            for i in range(len(carte[0]))
        ]
    ...
    retour = recherche_rec(M, depart, fin, depart)
    if retour:
        for elem in recup_solution(M, depart, fin)[1:]:
            var["chemin"].append(elem)
...
def recherche_rec(M, depart, fin, d_ou_je_viens):
	...
	:param list M: copie simplifié de la map afin d'indiquer les chemins
		deja effectué pour la recherche en cours
    ...
    M[y][x] = d_ou_je_viens
    if (x, y) == fin:
        var["chemin_trouve"] = True
        return True
    for direction_x, direction_y in direction:
        next_parcours = True
        if x + direction_x < 0 or y + direction_y < 0:
            next_parcours = False
        if x + direction_x > len(M[0]) or y + direction_y > len(M):
            next_parcours = False
        if M[y + direction_y][x + direction_x]:
            next_parcours = False
        if next_parcours:
            retour = recherche_rec(M, (x + direction_x, y + direction_y), fin, (x, y))
            if retour:
                return True
    return False


def recup_solution(M, debut, fin):
    """
	remonte les coordonnées inscrite dans chaque case indiquant a chaque fois de quelle case 
		il est venu remontant ainsi vers la position du personnage

	:param list M: copie simplifié de la map
	:param tuple debut: coordonnee de depart
	:param tuple fin: coordonnee d'arrivée
	:return: chemin trouvé pour aller du debut vers l'arrivée (pas le meilleur chemin (juste UN chemin)))
	"""
    chemin = [fin]
    while fin != debut:
        fin = M[fin[1]][fin[0]]
...
def recherche_parcours(carte, diamant):
    ...
    if not diamant:
        trouve = recherche_parcours_vers_position(
            carte,
            (var["pos_x"], var["pos_y"]),
            (var["pos_sortie_x"], var["pos_sortie_y"]),
        )
        if not trouve:
            return False

    pos_diamants = recup_pos_diamant_requis(carte)
    pos_diamants = sorted(
        pos_diamants,
        key=lambda coord: (
            (coord[0] - var["pos_x"]) ** 2 + (coord[1] - var["pos_y"]) ** 2
        )
        ** 0.5,
    )
    if pos_diamants:
        trouve = recherche_parcours_vers_position(
            carte, (var["pos_x"], var["pos_y"]), pos_diamants[0]
        )
        if not trouve:
            return False
...
def pathfinding(
    carte, nbdiamand, diamand, tempstotal, score, tempslumiere, position_suivante
):
    """
	recupere la position que doit effectuer le personnage et l'effectue
		Si un probleme (pierre non prevu a cause des éboulements) arrive, annule le pathfinding
		pour pouvoir par la suite rechercher un nouveau parcours

	:param list carte: carte du jeu
	:param int nbdiamant: nombre de diamant du personnage
	:param int diamand: diamant requis avant de pouvoir sortir
	:param int tempstotal: temps restant avant de perdre 
	:param int score: score du personnage en jeu
	:param int tempslumiere: temps restant avant d'etre dans le noir complet
	:param tuple position_suivante: contient la positions vers laquelle 
		le perso doit aller afin de se diriger doucement vers les diamants puis la sortie
	:return: nbdiamand, tempstotal, score, tempslumiere, deplacement_prevu_bien_realisé?
	"""
    x_actuel, y_actuel = var["pos_x"], var["pos_y"]
    x_suivant, y_suivant = position_suivante
    if x_suivant > x_actuel:
        x = "Right"
    elif x_suivant < x_actuel:
        x = "Left"
    elif y_suivant > y_actuel:
        x = "Down"
    else:
        x = "Up"
    ...
    if nbdiamand >= diamand and test_deplacement(carte, x, "E"):
        deplace(carte, x)
        return nbdiamand, tempstotal, score, tempslumiere, True
    elif (
        x == "Right"
