...
def recherche_parcours_vers_position(carte, depart, fin):
	M = [[None if carte[j][i] in {elem for elem in {".", "G", "L", "D", "R", "E"} if elem != "E" or fin == (var["pos_sortie_x"], var["pos_sortie_y"])}  else "WW" for i in range(len(carte[0]))] for j in range(len(carte))]
